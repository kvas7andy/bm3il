...
class LatentCEDisRNNAgent(nn.Module):
    ...
    def forward(self, inputs, hidden_state, t=0, batch=None, test_mode=None, t_glob=0, train_mode=False):
        ...
        latent_embed = self.latent.reshape(self.bs * self.n_agents, self.latent_dim * 2) # (bs * n, latent_dim *2)
        ...
        latent = gaussian_embed.rsample() # (bs * n, latent_dim *2)
        ...
        # Role -> FC2 Params
        latent = self.latent_net(latent) # (bs * n, NN_HIDDEN_SIZE)

        # role decoder
        fc2_w = self.fc2_w_nn(latent) # (bs * n, args.rnn_hidden_dim * args.n_actions)
        fc2_b = self.fc2_b_nn(latent) # (bs * n, args.n_actions)
