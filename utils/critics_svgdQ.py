...
class AttentionCritic(nn.Module):
    ...
    """
    def __init__(self, sa_sizes, hidden_dim=32, norm_in=True, critic_net_nums=1, attend_heads=1):
        """
        Inputs:
        ...
        self.critic_net_nums = critic_net_nums
    ...
    def scale_q_grads(self):
        """
        Scale gradients for critic since they are seperated into multiple pieces
        """
        for p in chain(*[self.critics[ai][qi].parameters() for ai in range(self.nagents) for qi in range(self.critic_net_nums)]):
            p.grad.data.mul_(1. * self.critic_net_nums)
    ...
    def svgd_q_grads(self):
        """
        compute svgd grads for shared parameters
        """
        for ai in range(self.nagents):
            dataDict = [[] for _ in range(self.critic_net_nums)]
            gradDict = [[] for _ in range(self.critic_net_nums)]
            for qi in range(self.critic_net_nums):
                for param in self.critics[ai][qi].parameters():
                    dataDict[qi].append(param.data)
                    gradDict[qi].append(param.grad)
            for idx, (w, grad) in enumerate(zip(zip(*dataDict), zip(*gradDict))):
                X = torch.stack(([wc.view(-1) for wc in w]),dim=0)
                score_func = torch.stack(([gc.view(-1) for gc in grad]),dim=0)
                svgdGrad = phi(score_func, RBF, X)
                for qi in range(self.critic_net_nums):
                    gradDict[qi][idx] = svgdGrad[qi].view(grad[qi].shape).data
            for qi in range(self.critic_net_nums):
                for param_cur, param_best in zip(self.critics[ai][qi].parameters(), gradDict[qi]):
                    param_cur.grad = param_best
    ...
    def forward(self, inps, agents=None, return_q=True, return_all_q=False,
        ...
        for i, a_i in enumerate(agents):
            ...
            all_q_list = [self.critics[a_i][qi](critic_in) for qi in range(self.critic_net_nums)]
            all_q = torch.mean(torch.stack(all_q_list),dim=0)
