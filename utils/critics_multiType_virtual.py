...
class AttentionCritic(nn.Module):
    ...
    def __init__(self, sa_sizes, type2agent, agent2type, hidden_dim=32, norm_in=True, attend_heads=1, attend_dim=None):
        ...
        #    sdim, adim = sa_sizes[self.type2agent[ti][0]]
        for sdim, adim in sa_sizes:
            # TBD, use a more systematic way to extract meanField info
            idim = sdim//2
            odim = adim
            ...
            self.mf_encoders.append(encoder)
            
            critic = nn.Sequential()
            critic.add_module('critic_fc1', nn.Linear(hidden_dim + attend_dim * attend_heads,
                                                      hidden_dim))
    ...
    def scale_shared_grads(self):
        ...
        for ti, module in enumerate(self.state_mf_encoders):
            for p in module.parameters():
                p.grad.data.mul_(1. / len(self.type2agent[ti]))
        for ti, module in enumerate(self.critics):
            for p in module.parameters():
                p.grad.data.mul_(1. / len(self.type2agent[ti]))
    ...
    def forward(self, inps, agents=None, return_q=True, return_all_q=False,
        ...
        # TBD, use a more systematic way to extract meanField info
        states_mf = [s[:,s.shape[1]//2:] for s in states]
        inps_mf = [states_mf[self.type2agent[ti][0]] for ti in range(self.ntypes)]
        # extract state-action encoding for each agent
        sa_encodings = [encoder(inp) for encoder, inp in zip(self.mf_encoders, inps_mf)]
        # extract state encoding for each agent that we're returning Q for
        s_encodings = [self.state_mf_encoders[self.agent2type[a_i]](states[a_i]) for a_i in agents]
        ...
        for curr_head_keys, curr_head_values, curr_head_selectors in zip(
            ...
            for i, a_i, selector in zip(range(len(agents)), agents, curr_head_selectors):
                keys = [k for j, k in enumerate(curr_head_keys) if j != self.agent2type[a_i]]
                values = [v for j, v in enumerate(curr_head_values) if j != self.agent2type[a_i]]
        ...
        for i, a_i in enumerate(agents):
            ...
            all_q = self.critics[self.agent2type[a_i]](critic_in)
