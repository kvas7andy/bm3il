...
class AttentionReward(nn.Module):
    ...
    """
    def __init__(self, sa_sizes, hidden_dim=32, norm_in=False, attend_heads=1, out_dim = 1, activation = None):
        """
        Inputs:
        ...
        """
        super(AttentionReward, self).__init__()
        ...
        for sdim, adim in sa_sizes:
            ...
            if activation:
                critic.add_module('activation', self.activation)
    ...
    def forward(self, inps, agents=None, return_q=True, return_all_q=False,
        ...
        for curr_head_keys, curr_head_values, curr_head_selectors in zip(
            ...
            for i, a_i, selector in zip(range(len(agents)), agents, curr_head_selectors):
                ...
                # calculate attention across agents. selector: (batch size, 1, selector length); keys: (batch size, key length, num of agents).
                # matmul works on highest dimensions. result attend_logits: (batch_size, 1, num of agents).
                attend_logits = torch.matmul(selector.view(selector.shape[0], 1, -1),
                                             torch.stack(keys).permute(1, 2, 0))
                # scale dot-products by size of key (from Attention is All You Need)
                scaled_attend_logits = attend_logits / np.sqrt(keys[0].shape[1])
                attend_weights = F.softmax(scaled_attend_logits, dim=2)
                # values: (batch size, value length, num of agents)
        ...
        for i, a_i in enumerate(agents):
            # concatenate all atention heads together
    ...
    def save(self, filename):
        ...
        """
        save_dict = {'critic': self.state_dict(),
