...
class AttentionCritic(nn.Module):
    ...
    """
    def __init__(self, sa_sizes, hidden_dim=32, norm_in=True, attend_heads=1, attend_dim=None):
        """
        Inputs:
        ...
        self.ntypes = len(sa_sizes)
        self.attend_heads = attend_heads

        self.input_mf_encoders = nn.ModuleList()
        self.critics = nn.ModuleList()

        self.mf_encoders = nn.ModuleList()
        ...
        for sdim, adim in sa_sizes:
            ...
            self.input_mf_encoders.append(encoder)
            ...
            mf_encoders = nn.Sequential()
            if norm_in:
                mf_encoders.add_module('s_enc_bn', nn.BatchNorm1d(
                                            sdim//2, affine=False))
            mf_encoders.add_module('s_enc_fc1', nn.Linear(sdim,
                                                            hidden_dim))
            mf_encoders.add_module('s_enc_nl', nn.LeakyReLU())
            self.mf_encoders.append(mf_encoders)
        ...
        self.shared_modules = [self.key_extractors, self.selector_extractors,
                               self.value_extractors, self.input_mf_encoders]
    ...
    def scale_shared_grads(self):
        ...
        for p in self.shared_parameters():
            p.grad.data.mul_(1. / self.ntypes)
    ...
    def forward(self, inps, agents=None, return_q=True, return_all_q=False,
        ...
        """
        if agents is None:
            agents = range(len(self.input_mf_encoders))
        states = [s for s, a in inps]
        actions = [a for s, a in inps]
        
        # TBD, use more systematic way for multi-type
        mfs = [s[:,s.shape[1]//2:] for s, a in inps]
        
        inps = [torch.cat((s, a), dim=1) for s, a in inps]
        # extract state-action encoding for each agent
        input_mf_encodings = [encoder(inp) for encoder, inp in zip(self.input_mf_encoders, inps)]
        # extract state encoding for each agent that we're returning Q for
        #mf_encodings = [self.mf_encoders[a_i](states[a_i]) for a_i in agents]
        # extract keys for each head for each agent
        #all_head_keys = [[k_ext(enc) for enc in input_mf_encodings] for k_ext in self.key_extractors]
        # extract sa values for each head for each agent
        #all_head_values = [[v_ext(enc) for enc in input_mf_encodings] for v_ext in self.value_extractors]
        # extract selectors for each head for each agent that we're returning Q for
        #all_head_selectors = [[sel_ext(enc) for i, enc in enumerate(mf_encodings) if i in agents]
        ...
        for i, a_i in enumerate(agents):
            ...
            agent_rets = []
            # only forward the self encoding to the critic
            critic_in = input_mf_encodings[i]
            ...
            if regularize:
                ...
                # TBD: for multi-type, need regularize
