...
from algorithms.attention_sac_cm_cmf_noAtt import AttentionSAC
...
class ARGS():
    def __init__(self):
        ...
        # save directories
        self.checkpoint_path = proj_direc + "buildILBerlinGAIL/data/" + self.env_name + "/checkpoint_GAILac3_cmf_noAtt_ds"
        self.description = 'GAILac'
        self.save_data_path = proj_direc+"buildILBerlinGAIL/data/"+self.env_name+"_GAILac3_cmf_noAtt_ds.pkl"
        self.save_vehicle_location_path = proj_direc+"buildILBerlinGAIL/data/"+self.env_name+"_GAILac3_cmf_noAtt_ds_vehicle_location"
        self.save_lane_vehicles_path = proj_direc+"buildILBerlinGAIL/data/"+self.env_name+"_GAILac3_cmf_noAtt_ds_lane_vehicles"
...
def update_params(sample, agentModels, discrimList):
    ...
    cur_mfacs, next_mfacs = agentModels.all_types_mean(sample)
    for _ in range(args.generator_epochs):
        agentModels.update_critic(sample,cur_mfacs,next_mfacs)
        agentModels.update_policies(sample,cur_mfacs)
