...
from algorithms.attention_sac_cs import AttentionSAC
...
class ARGS():
    def __init__(self):
        ...
        # save directories
        self.checkpoint_path = proj_direc + "buildILBerlinGAIL/data/" + self.env_name + "/checkpoint_GAILac1"
        self.description = 'GAILac'
        self.save_data_path = proj_direc+"buildILBerlinGAIL/data/"+self.env_name+"_GAILac1.pkl"
        self.save_vehicle_location_path = proj_direc+"buildILBerlinGAIL/data/"+self.env_name+"_GAILac1_vehicle_location"
        self.save_lane_vehicles_path = proj_direc+"buildILBerlinGAIL/data/"+self.env_name+"_GAILac1_lane_vehicles"
...
def create_networks():
    agentModels = AttentionSAC.init_from_env(env,
                                       mf_len=[0 for _ in range(len(env.agent2type))],
                                       tau=args.tau,
                                       pi_lr=args.pi_lr,
                                       q_lr=args.q_lr,
                                       gamma=args.gamma,
