...
from utils.critics_vanilla import AttentionCritic
...
class AttentionSAC(object):
    ...
    def step(self, observations, masks=None, explore=False):
        ...
        """
        #listSum = lambda x: torch.tensor([sum(y) for y in zip(*x)]) # list of tensor -> tensor of list element-wise sum
        #obsMF = [torch.tensor([sum(oi) for oi in zip(*[observations[ai] for ai in self.type2agent[ti]])]) for ti in range(self.ntypes)]
        #obsMF = [listSum([observations[ai] for ai in self.type2agent[ti]]) for ti in range(self.ntypes)]
        #return [self.agents[self.agent2type[ai]].step(torch.cat((obs,obsMF[agent2type[ai]]),1), mask, explore=explore) 
        #        for ai, obs, mask in zip(range(self.nagents), observations, masks)]
        #return [self.agents[self.agent2type[0]].step(observations[0], masks[0], explore=explore)]*self.nagents
        return [self.agents[self.agent2type[ai]].step(obs, mask, explore=explore) 
                 for ai, obs, mask in zip(range(self.nagents), observations, masks)]
    ...
    def update_critic(self, sample, soft=True, logger=None, **kwargs):
        ...
        #listSum = lambda x: torch.tensor([sum(y) for y in zip(*x)]) # list of tensor -> tensor of list element-wise sum
        #with torch.no_grad():
        #    obsMF = [[listSum(oi) for oi in zip(*[obs[ai] for ai in self.type2agent[ti]])] for ti in range(self.ntypes)]
        #    next_obsMF = [[listSum(oi) for oi in zip(*[next_obs[ai] for ai in self.type2agent[ti]])] for ti in range(self.ntypes)]
        #    for ai in range(self.nagents):
        #        obs[ai] = torch.cat((obs[ai],obsMF[agent2type[ai]]),1)
        #        next_obs[ai] = torch.cat((next_obs[ai],next_obsMF[agent2type[ai]]),1)
            
        # Q loss
        next_acs = []
        next_log_pis = []
        if self.policy_contain_mask:
            for ai, mask, ob in zip(range(self.nagents), masks, next_obs):
                curr_next_ac, curr_next_log_pi = self.target_policies[self.agent2type[ai]](ob, mask, return_log_pi=True)
                next_acs.append(curr_next_ac)
                next_log_pis.append(curr_next_log_pi)
        else:
            for ai, ob in zip(range(self.nagents), next_obs):
                curr_next_ac, curr_next_log_pi = self.target_policies[self.agent2type[ai]](ob, return_log_pi=True)
                next_acs.append(curr_next_ac)
                next_log_pis.append(curr_next_log_pi)
        trgt_critic_in = list(zip(next_obs, next_acs))
        critic_in = list(zip(obs, acs))
        next_qs = [self.target_critic([tci]) for tci in trgt_critic_in]
        critic_rets = [self.critic([ci], regularize=True,
                                  logger=logger, niter=self.niter) for ci in critic_in]
        q_loss = 0
        ...
        q_loss.backward()
        # TBD, multi type need to scale share grads
        #self.critic.scale_shared_grads()
        grad_norm = torch.nn.utils.clip_grad_norm_(
            self.critic.parameters(), 10 * self.ntypes)
    ...
    def update_policies(self, sample, soft=True, logger=None, **kwargs):
        ...
        if self.policy_contain_mask:
            for a_i, mask, ob in zip(range(self.nagents), masks, obs):
                curr_ac, probs, log_pi, pol_regs, ent = self.policies[self.agent2type[a_i]](
            ...
            for a_i, ob in zip(range(self.nagents), obs):
                curr_ac, probs, log_pi, pol_regs, ent = self.policies[self.agent2type[a_i]](
        ...
        critic_rets = [self.critic([ci], return_all_q=True) for ci in critic_in]
        for a_i, probs, log_pi, pol_regs, (q, all_q) in zip(range(self.nagents), all_probs,
                                                            all_log_pis, all_pol_regs,
                                                            critic_rets):
            curr_agent = self.agents[self.agent2type[a_i]]
    ...
    @classmethod
    def init_from_env(cls, env, mf_len, gamma=0.95, tau=0.01,
        ...
        for (typek,typev),typemfl in zip(env.type2agent.items(),mf_len):
            acsp = env.action_space[typev[0]]
            obsp = env.observation_space[typev[0]]
