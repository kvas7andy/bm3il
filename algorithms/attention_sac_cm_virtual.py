...
from utils.critics_multiType_virtual import AttentionCritic
...
class AttentionSAC(object):
    ...
    def __init__(self, agent_init_params, sa_size,
        ...
        Inputs:
            ...
            from cm to cm virtual (use one virtual agents):
                policy: change nagents, agents, policies, step, scale gradient
        """
    ...
    @property
    def policies(self):
        return [self.agents[self.agent2type[ai]].policy for ai in range(self.nagents)]

    @property
    def target_policies(self):
        return [self.agents[self.agent2type[ai]].target_policy for ai in range(self.nagents)]
    ...
    def update_policies(self, sample, soft=True, logger=None, **kwargs):
        ...
        for a_i, probs, log_pi, pol_regs, (q, all_q) in zip(range(self.nagents), all_probs,
            ...
            # CHANGE: use one virtual agent, down scale by number of agents in this type
            curr_agent.scale_shared_grads(len(self.type2agent[self.agent2type[a_i]]))
            grad_norm = torch.nn.utils.clip_grad_norm_(
