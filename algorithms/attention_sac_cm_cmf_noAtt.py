...
from utils.critics_multiType2_noAtt import AttentionCritic
...
class AttentionSAC(object):
    ...
    def all_types_mean(self, sample):
        obs, acs, mf_cur, masks, rews, next_obs, dones = sample
        next_acs = []
        for pi, mask, ob in zip(self.target_policies, masks, next_obs):
            curr_next_ac = pi(ob, mask)
        ...
        listMean = lambda x: np.array([sum(y)/len(y) for y in zip(*x)])
        next_acs_mftype = [[listMean(oi) for oi in zip(*[next_acs[ai].data.cpu().numpy() for ai in self.type2agent[ti]])] for ti in range(self.ntypes)]
        next_mfacs = cast(np.concatenate(next_acs_mftype,axis=1))
        cur_acs_mftype = [[listMean(oi) for oi in zip(*[acs[ai].data.cpu().numpy() for ai in self.type2agent[ti]])] for ti in range(self.ntypes)]
        cur_mfacs = cast(np.concatenate(cur_acs_mftype,axis=1))
        return cur_mfacs, next_mfacs
    
    def update_critic(self, sample, mf_cur, mf_next, soft=True, logger=None, **kwargs):
        """
        Update central critic for all agents
        """
        if self.policy_contain_mask:
            obs, acs, _, masks, rews, next_obs, dones = sample
        else:
            obs, acs, _, rews, next_obs, dones = sample
        # Q loss
        next_acs = []
        next_log_pis = []
        if self.policy_contain_mask:
        ...
        next_qs = self.target_critic(trgt_critic_in, mf_next)
        critic_rets = self.critic(critic_in, mf_cur, regularize=False,
    ...
    def update_policies(self, sample, mf_cur, soft=True, logger=None, **kwargs):
        if self.policy_contain_mask:
            obs, acs, _, masks, rews, next_obs, dones = sample
        else:
            obs, acs, _, rews, next_obs, dones = sample
        ...
        critic_rets = self.critic(critic_in, mf_cur, return_all_q=True)
