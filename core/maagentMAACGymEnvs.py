...
def collect_samples_thrd(numAgents, episode_length, index, mem_list, log_list, env, agentModels, custom_reward,
                    mean_action, render, running_state, min_batch_size):
    #torch.randn(ind)
    log = dict()
    log['reward_list'] = list()
...
class AgentsInteraction:
    ...
    def collect_samples(self, min_batch_size, episode_length, cuda, running_memory = None):
        ...
        mem_list = [None]*self.num_threads
        log_list = [None]*self.num_threads
        workers = []

        for i in range(self.num_threads-1):
            worker_args = (self.numAgents, episode_length, i+1, mem_list, log_list, self.env, self.agentModels, self.custom_reward, self.mean_action,
                           False, self.running_state, thread_batch_size)
            workers.append(thrd.Thread(target=collect_samples_thrd, args=worker_args))
        for worker in workers:
            worker.start()

        mem_list[0], log_list[0] = collect_samples_thrd(self.numAgents, episode_length, 0, None, None, self.env, self.agentModels, self.custom_reward, self.mean_action,
                                      self.render, self.running_state, thread_batch_size)
        for worker in workers:
            worker.join()

        # for _ in workers:
        #     index, worker_memory, worker_log = mem_log_list[]
        #     worker_memories[pid - 1] = worker_memory
        #     worker_logs[pid - 1] = worker_log
        for i in range(self.num_threads):
            for ai in range(self.numAgents):
                mem_list[0][ai].append(mem_list[i][ai])
        ...
        if running_memory:
            for ai in range(self.numAgents):
                running_memory[ai].append(mem_list[0][ai])
            batch = [running_memory[ai].sample() for ai in range(self.numAgents)]
        else:
            batch = [mem_list[0][ai].sample() for ai in range(self.numAgents)]
