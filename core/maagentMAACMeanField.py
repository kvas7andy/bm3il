...
from utils.replay_memory_MAAC_meanField import Memory
...
def collect_samples(numAgents, pid, queue, env, agentModels, custom_reward,
                    mean_action, render, running_state, min_batch_size, type2agent, agent2type):
    torch.randn(pid)
    log = dict()
    ...
    dtype = torch.float
    listMean = lambda x: np.array([sum(y)/len(y) for y in zip(*x)]) # list of np array -> array of list element-wise sum
    ntypes = len(type2agent)

    while num_steps < min_batch_size:
        state, valid_action_list = env.reset()
        #state
        #curMFType = [listMean([state[ai] for ai in type2agent[ti]]) for ti in range(ntypes)]
        #curMFAgent = [curMFType[agent2type[ai]] for ai in range(numAgents)]
        ...
        for t in range(10000):
            ...
            #action
            curMFType = [listMean([action[ai] for ai in type2agent[ti]]) for ti in range(ntypes)]
            curMFAgent = [curMFType[agent2type[ai]] for ai in range(numAgents)]
            
            next_state, reward, valid_action_list, done, _ = env.step(action)
            
            #state
            #nextMFType = [listMean([next_state[ai] for ai in type2agent[ti]]) for ti in range(ntypes)]
            #nextMFAgent = [nextMFType[agent2type[ai]] for ai in range(numAgents)]
            #action
            #torch_next_obs = [tensor(st).to(dtype).unsqueeze(0) for st in next_state]
            #with torch.no_grad():
            #    torch_next_action = agentModels.target_step(torch_next_obs, masks=torch_masks, explore=True)
            #    next_action = [ac.data.numpy()[0] for ac in torch_next_action]
            #nextMFType = [listMean([next_action[ai] for ai in type2agent[ti]]) for ti in range(ntypes)]
            #nextMFAgent = [nextMFType[agent2type[ai]] for ai in range(numAgents)]
            ...
            for ai in range(numAgents):
                memory[ai].push(state[ai], action[ai], curMFAgent[ai], valid_action_list[ai], reward[ai], next_state[ai], done)
...
class AgentsInteraction:
    ...
    def __init__(self, env, numAgents, agentModels, device, custom_reward=None,
                 mean_action=False, render=False, running_state=None, num_threads=1, type2agent=None, agent2type=None):
        self.env = env
    ...
    def batch2TensorSample(self,batch,cuda):
        ...
        obs, acs, mf_cur, masks, rews, next_obs, dones = [], [], [], [], [], [], []
        for sampleAi in batch:
            obsAi, acsAi, mf_curAi, masksAi, rewsAi, next_obsAi, donesAi = sampleAi
            obs.append(cast(obsAi))
            acs.append(cast(acsAi))
            mf_cur.append(cast(mf_curAi))
            masks.append(cast(masksAi))
            rews.append(cast(rewsAi))
            next_obs.append(cast(next_obsAi))
            dones.append(cast(donesAi))
        return([obs, acs, mf_cur, masks, rews, next_obs, dones])
    ...
    def collect_samples(self, min_batch_size, cuda):
        ...
        for i in range(self.num_threads-1):
            worker_args = (self.numAgents, i+1, queue, self.env, self.agentModels, self.custom_reward, self.mean_action,
                           False, self.running_state, thread_batch_size, self.type2agent, self.agent2type)
        ...
        memory, log = collect_samples(self.numAgents, 0, None, self.env, self.agentModels, self.custom_reward, self.mean_action,
                                      self.render, self.running_state, thread_batch_size, self.type2agent, self.agent2type)
